app.controller('searchController',function($scope ,$location ,searchService){
	$scope.mapSearch = {'keywords':'','category':'','brand':'','spec':{},'priceRange':'','pageIndex':1,'pageSize':30,'sortWay':'','sortField':''};
	searchRecords = [];
	...
	$scope.sortSearch = function(sortWay,sortField){
		$scope.mapSearch.sortWay = sortWay;
		$scope.mapSearch.sortField = sortField;
		$scope.search(true);
	}
	//判断搜索内容是否包含品牌
	$scope.isKeywordsContainBrand = function(){
		for(var i=0;i<$scope.resultMap.brandList.length;i++){
			if($scope.mapSearch.keywords.indexOf($scope.resultMap.brandList[i].text) >= 0){
				return true;
			}
		}
		return false;
	...
	$scope.receiveKeywords = function(){
		$scope.mapSearch.keywords = $location.search()['keywords'];
		if($scope.mapSearch.keywords != null)
			$scope.search(true);
